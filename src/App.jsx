...
const MODULES = [
  ...
  {
    id: 2,
    title: "Profile Khách Công Ty",
    subtitle: "B2B — VAT, Công nợ & Hạn mức",
    icon: "🏢",
    color: "#8b5cf6",
    tag: "B2B",
    lessons: [
      {
        ...
        content: [
          ...
          { type: "intro", text: "Các công ty cần nhập đầy đủ thông tin để phục vụ xuất hoá đơn và quản lý công nợ." },
          { type: "divider", text: "Thông tin cơ bản" },
          { type: "field", label: "Address", desc: "Địa chỉ đầy đủ: Street, City, ZIP, State, Country." },
          { type: "field", label: "Tax ID", desc: "Mã số thuế — bắt buộc để xuất hoá đơn VAT." },
          { type: "field", label: "Phone / Mobile", desc: "Số điện thoại cố định và di động." },
          { type: "field", label: "Email", desc: "Email công ty — gửi invoice và xác nhận booking." },
          { type: "field", label: "Website", desc: "Website của công ty (không bắt buộc)." },
          { type: "field", label: "Language", desc: "Ngôn ngữ giao tiếp (mặc định: English US)." },
          { type: "field", label: "Tags", desc: "Nhãn phân loại: B2B, VIP, Consulting..." },
          { type: "point", icon: "➕", title: "Contacts & Addresses", desc: "Thêm danh sách contact người đại diện của công ty." },
          { type: "point", icon: "💰", title: "Sales & Invoiced", desc: "Tổng số sales và tổng số tiền đã invoiced." },
        ],
        hasDemo: false,
      },
      {
        ...
        content: [
          ...
          { type: "field", label: "Salesperson", desc: "Nhân viên sales phụ trách công ty này." },
          { type: "field", label: "Payment Terms", desc: "Thời hạn thanh toán (VD: 7 Days)." },
          { type: "field", label: "Payment Method", desc: "Hình thức thanh toán (VD: Manual Payment Bank)." },
          { type: "divider", text: "AGENT" },
          { type: "field", label: "Agent", desc: "Checkbox — đánh dấu nếu là agent booking." },
          { type: "field", label: "Is OTA", desc: "Checkbox — đánh dấu nếu là kênh OTA." },
          { type: "field", label: "Is MVillage Issue", desc: "Checkbox — đánh dấu nếu là issue nội bộ MVillage." },
          { type: "field", label: "Agent in charge", desc: "Nhân viên phụ trách agent này." },
          { type: "warn", text: "Payment Terms và Payment Method ảnh hưởng đến quy trình thanh toán công nợ. Cần nhập đúng trước khi tạo booking B2B." },
          { type: "divider", text: "QUY ĐỊNH CÔNG NỢ B2B" },
          { type: "steps", items: [
            "Sale tạo profile công ty lên hệ thống và nhập đầy đủ thông tin.",
            "Sau khi tạo xong, Sale liên hệ Kế toán để xin cấp mã khách (Account Receivable Code).",
            "Kế toán cấp mã khách và nhập vào profile công ty — từ đó công ty được phép công nợ trên hệ thống.",
          ]},
          { type: "card2", items: [
            { title: "✅ Có mã khách (AR Code)", desc: "Công ty được phép công nợ. Balance có thể > 0 khi checkout. Thanh toán sau theo Payment Terms đã thoả thuận." },
            { title: "❌ Chưa có mã khách", desc: "Công ty KHÔNG được phép công nợ. Bắt buộc phải thanh toán 100% trước thời điểm Checkin." },
          ]},
        ],
        hasDemo: true,
      },
    ...
    lessons: [
      {
        ...
        content: [
          ...
          { type: "intro", text: "Một số khái niệm và cách hoạt động được thay đổi so với hệ thống cũ. Team vận hành cần nắm rõ trước khi tạo booking." },
          { type: "divider", text: "Booking Type" },
          { type: "point", icon: "🔄", title: "Không còn màn hình tạo Long Stay riêng", desc: "Hệ thống mới sử dụng chung 1 màn hình tạo booking cho tất cả loại hình lưu trú." },
          { type: "point", icon: "☑️", title: "Phân biệt bằng Checkbox", desc: "Tích chọn Is Long Stay hoặc Is Mid Stay để phân loại loại hình lưu trú ngay trên màn hình booking." },
          { type: "card2", items: [
            { title: "Is Mid Stay", desc: "Đánh dấu khi khách lưu trú theo hình thức Mid Stay (lưu trú trung hạn)." },
            { title: "Is Long Stay", desc: "Đánh dấu khi khách lưu trú theo hình thức Long Stay (lưu trú dài hạn)." },
          ]},
          { type: "divider", text: "Service Start — Service End" },
          { type: "point", icon: "📅", title: "Service Start / Service End", desc: "Quy định ngày sử dụng của từng dịch vụ. Mỗi dịch vụ (phòng, BF, minibar...) đều có Service Start và Service End riêng — xác định chính xác thời điểm ghi nhận doanh thu." },
          { type: "divider", text: "Actual Checkin — Checkout" },
          { type: "point", icon: "🕐", title: "Actual Checkin - Checkout", desc: "Ngày giờ checkin và checkout THỰC TẾ của khách — được hệ thống ghi nhận tự động khi Ops bấm nút Checkin/Checkout. Khác với Expected Checkin-Checkout là ngày giờ dự kiến khi tạo booking." },
          { type: "divider", text: "Folio" },
          { type: "point", icon: "💼", title: "Guest Folio", desc: "Guest Folio là công cụ quản lý TẤT CẢ các khoản charge và payment của khách lưu trú. Mỗi reservation tự động tạo 1 Folio chính theo main guest. Sau khi Checkin, mọi thao tác (phát sinh, thanh toán) đều thực hiện trên Folio." },
          { type: "divider", text: "Deposit" },
          { type: "point", icon: "💰", title: "Deposit", desc: "Ghi nhận thanh toán trước checkin. Deposit được thu để đảm bảo khách có khả năng thanh toán — bắt buộc 100% trước thời điểm Checkin." },
        ],
        hasDemo: false,
      },
      {
        ...
        content: [
          ...
          { type: "divider", text: "Action Buttons" },
          { type: "point", icon: "💰", title: "Deposit", desc: "Thu tiền đặt cọc — chỉ thực hiện được khi booking ở trạng thái Confirm." },
          { type: "point", icon: "❌", title: "Cancel Reservation", desc: "Huỷ booking và release inventory phòng." },
          { type: "point", icon: "🔑", title: "Checkin", desc: "Thực hiện checkin chính thức cho khách." },
          { type: "point", icon: "🚪", title: "Change Room", desc: "Chuyển phòng khi khách đang trong trạng thái Checkin." },
          { type: "point", icon: "🚫", title: "No Show", desc: "Xử lý khi khách không đến theo booking." },
          { type: "divider", text: "Trạng thái Booking" },
          { type: "card2", items: [
            { title: "Draft", desc: "Booking mới tạo, chưa xác nhận. Chưa thể add deposit." },
            { title: "Confirm", desc: "Booking đã xác nhận. Bắt đầu add deposit và chuẩn bị checkin." },
          ]},
          { type: "divider", text: "Thông tin chính" },
          { type: "field", label: "Booker", desc: "Tên người đặt phòng." },
          { type: "field", label: "Date Ordered", desc: "Ngày giờ tạo booking." },
          { type: "field", label: "Hotel", desc: "Chi nhánh khách sạn." },
          { type: "field", label: "Pricelist", desc: "Bảng giá áp dụng cho booking này." },
          { type: "field", label: "Source", desc: "Nguồn booking (Internal Reservation, OTA, Walkin...)." },
          { type: "field", label: "Group ID", desc: "Mã nhóm — điền khi khách đi nhóm có nhiều phòng." },
          { type: "field", label: "Note", desc: "Ghi chú thêm cho booking." },
          { type: "field", label: "Person", desc: "Số lượng người: Adults, Children, Young Child, Babies." },
          { type: "field", label: "VAT?", desc: "Checkbox — tích nếu khách cần xuất hoá đơn VAT." },
          { type: "field", label: "Via", desc: "Kênh đặt phòng (Direct, OTA...)." },
          { type: "field", label: "Direct Channel", desc: "Kênh trực tiếp cụ thể (BOD, OPS, FO...)." },
          { type: "field", label: "Purpose of stay", desc: "Mục đích lưu trú của khách." },
          { type: "field", label: "Is Mid Stay / Is Long Stay", desc: "Checkbox phân loại loại hình lưu trú." },
          { type: "field", label: "Actual Loyalty Nights", desc: "Số đêm loyalty thực tế tích lũy." },
        ],
        hasDemo: false,
      },
      {
        ...
        content: [
          ...
          { type: "divider", text: "Thông tin thời gian" },
          { type: "field", label: "Expected Checkin - Checkout", desc: "Ngày giờ dự kiến checkin và checkout khi tạo booking." },
          { type: "field", label: "Actual Checkin - Checkout", desc: "Ngày giờ thực tế — tự động cập nhật khi Ops bấm Checkin/Checkout." },
          { type: "divider", text: "Bảng Room Booking" },
          { type: "field", label: "Guest Name", desc: "Tên khách lưu trú trong phòng." },
          { type: "field", label: "Status", desc: "Trạng thái xác thực giấy tờ (Not Signed, Verified...)." },
          { type: "field", label: "Room Type / Room Number", desc: "Loại phòng và số phòng cụ thể." },
          { type: "field", label: "Service Start / Service End", desc: "Ngày giờ bắt đầu và kết thúc sử dụng phòng." },
          { type: "field", label: "Number of nights", desc: "Số đêm lưu trú." },
          { type: "field", label: "Price / Taxes / Subtotal", desc: "Giá phòng, thuế, tổng tiền sau thuế." },
          { type: "divider", text: "Các Tab khác" },
          { type: "point", icon: "📋", title: "Rate Plan", desc: "Xem chi tiết gói giá đang áp dụng cho booking." },
          { type: "point", icon: "➕", title: "Add-on Services", desc: "Các dịch vụ bán kèm theo booking (BF, airport transfer...)." },
          { type: "point", icon: "💰", title: "Deposit Details", desc: "Lịch sử các giao dịch deposit của booking này." },
          { type: "point", icon: "👥", title: "Guest in room", desc: "Danh sách tất cả khách trong phòng (main guest + sharers)." },
          { type: "warn", text: "Sau khi Checkin, booking bị KHOÁ. Mọi thao tác (charge, payment) phải thực hiện trên FOLIO." },
        ],
        hasDemo: true,
      },
      {
        ...
        imageCaption: "Advance Service Lines — danh sách dịch vụ bán kèm theo booking: BF, Pick up/Drop off, Promotion",
        content: [
          { type: "heading", text: "Advance Service Lines" },
          { type: "intro", text: "Advance Service Lines là các dịch vụ bán kèm theo booking trước khi checkin (BF, airport transfer...). Mỗi dịch vụ đều có Service Start và Service End quy định ngày sử dụng thực tế." },
          { type: "divider", text: "Các cột thông tin" },
          { type: "field", label: "Product", desc: "Tên dịch vụ (TTD Ăn sáng - 1Pax, Pick up/Drop off...)." },
          { type: "field", label: "Description", desc: "Mô tả chi tiết dịch vụ." },
          { type: "field", label: "Category", desc: "Nhóm phân loại dịch vụ." },
          { type: "field", label: "Quantity", desc: "Số lượng dịch vụ." },
          { type: "field", label: "Unit Price", desc: "Đơn giá chưa thuế." },
          { type: "field", label: "Service Start / Service End", desc: "Ngày giờ bắt đầu và kết thúc sử dụng dịch vụ — xác định chính xác thời điểm ghi nhận doanh thu." },
          { type: "field", label: "Number of nights", desc: "Số đêm áp dụng (với dịch vụ tính theo đêm)." },
          { type: "field", label: "Taxes", desc: "Thuế áp dụng (VD: 8% - 29%)." },
          { type: "field", label: "Disc.%", desc: "Phần trăm giảm giá." },
          { type: "field", label: "Subtotal", desc: "Tổng tiền sau thuế và giảm giá." },
          { type: "divider", text: "Quy định Service Start — Service End" },
          { type: "card2", items: [
            { title: "Ăn sáng (BF)", desc: "Tách từng dòng tương ứng với từng ngày sử dụng.\nVD: LOS 2 đêm → 2 dòng BF, mỗi dòng 1 ngày riêng." },
            { title: "Spa / Minibar / Dịch vụ trong ngày", desc: "Quy định khung giờ sử dụng cụ thể.\nVD: Minibar 05/09 14:00 → 05/10 12:00." },
          ]},
          { type: "warn", text: "Service Start/End ảnh hưởng trực tiếp đến thời điểm ghi nhận doanh thu khi chạy Night Audit. Nhập sai sẽ gây sai lệch báo cáo." },
          { type: "divider", text: "PROMOTION" },
          { type: "point", icon: "🏷️", title: "Promotion Code", desc: "Nhập mã khuyến mãi nếu có. Hệ thống tự động tính Discount Value tương ứng." },
        ],
        hasDemo: false,
      },
      {
      ...
      },
      {
        ...
        content: [
          { type: "heading", text: "Các Thao Tác Được Cải Thiện — Truy Cập Nhanh Hơn" },
          { type: "intro", text: "NewPMS Odoo 18 cải thiện một số thao tác giúp Ops thực hiện nhanh hơn mà không cần navigate qua nhiều bước." },
